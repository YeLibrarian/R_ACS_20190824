...
There are `r length(benzoate$pka)` values and the median value is `r median(benzoate$pka)`.

### Plots

```{r boxplot}
boxplot(benzoate$pka, ylab="pKa values")
```

```{r pKa_histogram}
hist(benzoate$pka, main="Distribution of pKa Values", xlab="pKa Values")
```
...
plot(benzoate$pka, fitted(model.5), main="This is the title", xlab="Observed pKa", ylab="Predicted pKa")
```
...
3.3 
For _o_-Hydroxybenzoic acid the actual pKa is 2.98, but the model predicted a pKa of 
